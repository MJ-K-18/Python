'''
    training11_2_scoretable_Inheritance_3layer_file.py
'''
...
class DepartmentTable:
    '''
        ...
        method : 생성자( __init__() )
                 학과 정보 Load( loadDepartmentInfo() )
                 학과 정보 Save( saveDepartmentInfo() )
                 학과 정보 읽기( readDepartmentInfo() )
    ...
    def __init__( self ):
        ...
        self.loadDepartmentInfo()

    def loadDepartmentInfo( self ):
        try:
            with open( 'departmentinfo.txt', 'r', encoding = 'utf-8' ) as f:
                r = f.readline()
                while r:
                    l = r.split( ',' )
                    self.departments[ l[ 0 ] ] = DepartmentInfo( l[ 0 ], int( l[ 1 ] ) )
                    self.departmentCount += 1 
                    r = f.readline()

        except FileNotFoundError:
            with open( 'departmentinfo.txt', 'w', encoding = 'utf-8' ) as f:
                pass

    def saveDepartmentInfo( self ):
        with open( 'departmentinfo.txt', 'w', encoding = 'utf-8' ) as f:
            for key in self.departments:
                department = self.departments[ key ]
                r = '{0},{1},\n'.format( department.getDepartmentName(), department.getMajorSubjectMax() )
                f.write( r )
...
class ScoreTable:
    '''
        ...
        method : 생성자( __init__() )
                 학생 정보 Load( loadStudentInfo() )
                 학생 정보 Save( saveStudentInfo() )
                 성적일람표 Save( saveScoreTable() )
            ...
            with open( 'studentinfo.txt', 'r', encoding = 'utf-8' ) as f:
                ...
                while r:
                    ...
                    for _ in range( int( l[ 2 ] ) ):
                        ...
                        index += 1
                    student = DepartmentStudentInfo( l[ 0 ], l[ 1 ], int( l[ 2 ] ), c, m )
    ...
    def saveStudentInfo( self ):
        with open( 'studentinfo.txt', 'w', encoding = 'utf-8' ) as f:
            for key in self.scoretable:
                student = self.scoretable[ key ]
                r = '{0},{1},{2},'.format( student.getName(), student.getDepartmentName(), student.getMajorSubjectMax() )
                for v in student.getCommonSubjects():
                    r = r + '{0},'.format( v )

                for v in student.getMajorSubjects():
                    r = r + '{0},'.format( v )
                r = r + '\n'
                f.write( r )
...
class ScoreTableUI:
    '''
        ...
        method : 생성자( __init__() )
                 ...
                 성적일람표를 출력한다.( printScoreTable() )
                 성적일람표를 파일에 기록하다.( saveScoreTable() )
    '''
    def __init__( self ):
...
2. 다음과 같이 출력하는 프로그램을 class를 작성하여 완성하세요.
   ...
   관리하고, 파일에 학생 정보와 성적일람표를 저장한다.
    ...
    def inputDepartmentInfo( self ):
        ...
        while select.upper() == 'Y':
            ...
            while ( select.upper() != 'Y' and select.upper() != 'N' ):
                print( '\nError : Y 또는 N만 입력 가능합니다.' )
                select = input( '학과를 추가 하시겠습니까? (Y/N) : ' )
        self.departmentTable.saveDepartmentInfo()
    ...
    def inputStudentInfo( self ):
        self.name = input( '\n{0:2}번 학생 이름 입력 ( "end" : quit ) : '.format( ScoreTable.count_student + 1 ) )
        while  self.name != 'end' and  ScoreTable.count_student < ScoreTable.MAX:
    ...
    def printScoreTable( self ):
        ...
        self.saveScoreTable()
        self.scoreTable.saveStudentInfo()

        input( '\nPress any key to program stop...' ) 

    def saveScoreTable( self ):
        with open( 'scoretable.txt', 'w', encoding = 'utf-8' ) as f:
            self.studentInfos = self.scoreTable.printScoreTable()
            for studentInfo in self.studentInfos:
                s = str( studentInfo )
                f.write( s )
